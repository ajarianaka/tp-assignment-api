...
export class BaseController {
    ...
    */
     ...
     */
    findById(res: Response, documentId: string, populate?: IPopulate, select?: ISelect, errMsg = `Failed to find document ${documentId}`) {
        this.model.findById(documentId, populate, select).then(doc => { this.jsonRes(doc, res) }, err => { this.errRes(err, res, errMsg) }).catch(err => { this.errRes(err, res, 'Failed to retrieve doc') });
    }
    /**
     ...
     */
    updateById(res: Response, documentId: string, document: any, populate?: IPopulate | IPopulate[], select?: ISelect, errMsg = `Failed to update document ${documentId}`) {
        this.model.updateById(documentId, document, populate, select).then(doc => { this.jsonRes(doc, res) }, err => { this.errRes(err, res, errMsg) });
    }
    /**
